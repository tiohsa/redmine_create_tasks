...
- 左側のプラスハンドルは依存元タスクの追加に使います。チケット登録時は、依存元から対象タスクへの Redmine `precedes` 関係を作成します。
- 既存ノードを別ノードの左側プラスハンドルへドラッグすると依存関係を作成します。
- 既存ノードを別ノードの下側プラスハンドルへドラッグすると子チケットとして移動します。
- 依存関係と親子階層は同時に登録されます。グローバルな関連付けモード切替はありません。
